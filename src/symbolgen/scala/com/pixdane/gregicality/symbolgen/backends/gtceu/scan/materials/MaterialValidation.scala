...
/** Turns material scan inputs into ordered GTCEu diagnostics.
  *
  * Each method handles one diagnostic class (duplicate assignments, duplicate
  * registry ids, rejected builders, missing assignments) and returns them
  * deterministically sorted so that output is stable across runs.
  */
object MaterialValidation:
